...
def build_cds_pangenome(genome_faa_paths, output_dir, name='Test', 
    ...
    output_nr_faa = output_dir + '/' + name + '' # final non-redundant FAA files
    ...
    output_nr_faa_copy = output_nr_faa + '' # temporary FAA copy generated by CD-Hit
    output_nr_clstr = output_nr_faa + '.clstr' # clusteeipxzr file generated by CD-Hit
...
# Generate submatrices to measure robustness
def get_gene_frequency_submatrices(P, breakpoints=[0, 25, 50, 75, 100]):
    # Convert the matrix P into a DataFrame if it isn't already
    if not isinstance(P, pd.DataFrame):
        P = pd.DataFrame(P)
    
    # Calculate the gene frequencies as percentages
    gene_frequencies = P.sum(axis=1) / P.shape[1] * 100
    
    # Initialize a nested dictionary to store the submatrices
    result = {min_freq: {max_freq: None for max_freq in breakpoints} for min_freq in breakpoints}
    
    for min_freq in breakpoints:
        for max_freq in breakpoints:
            if min_freq < max_freq:
                submatrix = P[(gene_frequencies > min_freq) & (gene_frequencies <= max_freq)]
                result[min_freq][max_freq] = submatrix
            else:
                result[min_freq][max_freq] = pd.DataFrame() # empty DataFrame
...
# Generate connectivity / adjacency matrix for samples
...
def fit_bi_modal(data):
    # Reshape data for modeling
    data_reshaped = np.array(data).reshape(-1, 1)
    
    # Fit a Gaussian Mixture Model with 2 components
    gmm = GaussianMixture(n_components=2, random_state=0).fit(data_reshaped)
    
    # Extract the means of the two components
    means = gmm.means_.flatten()  # Flatten to convert from 2D to 1D array
    
    # Extract the standard deviations of the two components
    std_devs = np.sqrt(gmm.covariances_.flatten())  # Diagonal for 1D data, square root for std deviation
    
    return means, std_devs


def detect_outliers_modified_zscore(data, threshold=3):
    ...
    # Outliers are those with a modified Z-score greater than the threshold and less than the median
    outliers = (modified_z_scores > threshold) & (data < median_data)
    return outliers


def iteratively_filter_outliers(data_series):
    filtered_data = data_series.copy()
    all_outliers = pd.Series(dtype=float)
    
    while True:
        outliers_mask = detect_outliers_modified_zscore(filtered_data, threshold=3)
        if not outliers_mask.any():
            break
        # Collect outliers
        current_outliers = filtered_data[outliers_mask]
        all_outliers = pd.concat([all_outliers, current_outliers])
        # Remove outliers
        filtered_data = filtered_data.loc[~outliers_mask]
    
    return filtered_data, all_outliers
